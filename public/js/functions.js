...
$document.on("ready", function () {
	...
	formCTASubscribe1();
	formCTASubscribe2(); // wurde für 2. CTA-Form ergänzt
	scrollToAnchor();
	counterStats();
	// sliderClients();
	// sliderTestimonials();
...
function sectionParallaxImageBG() {
	$(".section-parallax").each(function () {
		...
		bgElement
			.css("background-image", "url('" + imgSrc + "')")
			.attr("data-stellar-background-ratio", 0.1);
	});
}
...
function cs1Success() {
	var s = csNotifications.data("success-msg"),
		i = s || "Thank you for your submission :)";
...
function counterStats() {
	$(".counter-stats").each(function () {
		...
		t.html(a + "<div class='main'>" + i + "</span>"),
			t.find(".digit").each(function () {
				var t = $(this),
					i = t.find(".digit-value").text();
				t.append(
							...
							1 < $(window).width() &&
								a.find(".counter-animator").each(function () {
									...
									t.find("ul").css({
										...
										"-webkit-transform": "translateY(-" + i + "%)",
										"-moz-transform": "translateY(-" + i + "%)",
										"-ms-transform": "translateY(-" + i + "%)",
										"-o-transform": "translateY(-" + i + "%)",
									});
								});
						}, 0 * i);
					}, 0);
...
// *** Slider Clients *** //
// *** Derzeit auskommentiert da nicht benötigt +++ //
